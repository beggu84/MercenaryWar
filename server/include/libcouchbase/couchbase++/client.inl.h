...
inline Client::Client(const std::string& connstr, const std::string& passwd)
...
inline void GetResponse::assign_first(const GetResponse& other) {
    u.resp = other.u.resp;
    if (has_shared_buffer()) {
        lcb_backbuf_ref((lcb_BACKBUF)u.resp.bufh);
    } else if (has_alloc_buffer()) {
...
ObserveResponse::handle_response(Client&, int, const lcb_RESPBASE *res)
{
    ...
    if (res->rc == LCB_SUCCESS) {
        ...
        r.cas = res->cas;
        r.status = ro->status;
        r.master = (ro->ismaster != 0);
        sinfo.push_back(r);
    }
}

inline const ObserveResponse::ServerReply&
...
inline GetResponse::GetResponse() : Response() {
    u.resp.bufh = NULL;
    u.resp.value = NULL;
    u.resp.nvalue = 0;
}
